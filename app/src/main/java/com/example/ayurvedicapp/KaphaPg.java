...
public class KaphaPg extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView kaphaInfoTextView=findViewById(R.id.kaphaInfoTextView);
        ...
        SpannableString spannableContent=new SpannableString(kaphinfo);

        String[] boldTexts={"Kapha nirukti","Structure of Kapha dosh:","Properties of Kapha dosh:","Physical Symptoms:","Emotional and Mental Symptoms:","Effects of Prolonged Imbalance:","Kapha Cures:"};
        for(String boldText:boldTexts){
            int start=kaphinfo.indexOf(boldText);
            if(start>=0){
                spannableContent.setSpan(new StyleSpan(Typeface.BOLD),start,start+boldText.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }

        kaphaInfoTextView.setText(spannableContent);
        kaphaInfoTextView.setJustificationMode(LineBreaker.JUSTIFICATION_MODE_INTER_WORD);

        ArrayList<Symptomodel> arrkapha=dBhelper.fetchdatkaphainfo();
        for(Symptomodel model: arrkapha){
            spannableContent=new SpannableString(model.Kapha);
            kaphaInfoTextView.append("\n\n"+spannableContent);
        }
