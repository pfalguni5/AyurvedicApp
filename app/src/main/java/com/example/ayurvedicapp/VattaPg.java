...
public class VattaPg extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listViewInfo=findViewById(R.id.listVatta);
        ArrayList<String> vattinfo = new ArrayList<>();
        ...
        if (!dBhelper.isDataExistvatinfo(vatinfo)) {
            dBhelper.addDatainfo(vatinfo);
        }

        ArrayList<Symptomodel> arrinfo = dBhelper.fetchdatinfo();

        for (Symptomodel model : arrinfo) {
            vattinfo.add(model.Info);
        }


        ArrayAdapter<String> adapter = new ArrayAdapter<>(this, R.layout.textcolor, R.id.textViewItem, vattinfo);
